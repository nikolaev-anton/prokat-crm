...
$allowed_keys = array("order_id", "taker_id");
...
if(!empty($request["taker_id"]))
	{
	$taker_sql = ",taker_id=".$request["taker_id"];
	}
else
	{
	$taker_sql="";
...
if (!$result) 
  {
  finish("take_order UPDATE error: ". mysqli_sqlstate($db_conection) . mysqli_error($db_conection));
  };
mysqli_free_result($result);


//перемещение товаров из виртуального склада "в прокате" в место возврата
$sql = 
"SELECT 
  TAKE_STOCK_ID 
FROM 
...
$take_stock_id = intval($row["TAKE_STOCK_ID"]);
mysqli_free_result($result);
...
foreach(array_keys($model_counts) as $model_id)
	...
	$sql = "select ID from ITEMS where STOCK_ID=".$client_stock_id." and MODEL_ID=".$model_id." LIMIT ".$model_counts[$model_id];
	$result = mysqli_query($db_conection, $sql);
	if (!$result) 
		{
	...
	$sql = "update ITEMS set STOCK_ID=".$take_stock_id." where ID in (" . implode(",",$items) . ")";
	$result = mysqli_query($db_conection, $sql);
	if (!$result) 
		{
	...
	for($i=count($items); $i>0; $i--)
		{
		$sql = $sql . "('giving order # ".$order_id."',".$client_stock_id.",".$take_stock_id.",".$items[$i-1].")";
		if($i <> 1)
			{
			$sql = $sql . ",";
			}
